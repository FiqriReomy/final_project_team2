        ...
        {getProducts.map((product) => (
          <Link key={product.id} to={`/product/${product.id}`} className="flex items-start hover:no-underline">
            <div className="w-10 h-10 min-w-[2.5rem] bg-gray-200 rounded-sm">
              <img className="w-full h-full object-cover rounded-sm" src={product.url} alt={product.product_name} />
            </div>
            <div className="ml-4 flex-1">
              <p className="text-sm text-gray-800">{product.name}</p>
              <span className={classNames(product.qty === 0 ? "text-red-500" : product.qty > 50 ? "text-green-500" : "text-orange-500", "text-xs font-medium")}>
                {product.qty === 0 ? "Out of Stock" : product.product_stock + " in Stock"}
              </span>
            </div>
